...
{
    private const string UiBehaviorCheckersScenePath = "res://addons/BehaviorAI/UI/UiBehaviorCheckers.tscn";
    private const string UiBehaviorActionScenePath = "res://addons/BehaviorAI/UI/UiBehaviorAction.tscn";
    ...
    public override void _Ready()
    {
        ...
        _signalOption.Clear();
        foreach (var (signal, id) in BehaviorAI.Behavior.SignalName2Id)
        {
            _signalOption.AddItem(signal, id);
        }

        _signalOption.Select(string.IsNullOrEmpty(BehaviorUnit?.Signal) ? -1 : _signalOption.GetItemIndex(BehaviorAI.Behavior.SignalName2Id[BehaviorUnit.Signal]));
    ...
    private void OnSignalOptionItemSelected(long index)
    {
        ...
        BehaviorUnit.Signal = BehaviorAI.Behavior.SignalId2Name[id];
    }
    ...
    private void OnAddActionPressed(BehaviorAction action = null)
    {
        ...
        uiBehaviorAction.Action = action;
