...
public partial class UiBehaviorUnit : PanelContainer
{
    ...
    public override void _Ready()
    {
        ...
        _signalOption.Clear();
        foreach (var (signal, id) in Core.Behavior.SignalName2Id)
        {
            _signalOption.AddItem(signal, id);
        }

        _signalOption.Select(string.IsNullOrEmpty(BehaviorUnit?.Signal) ? -1 : _signalOption.GetItemIndex(Core.Behavior.SignalName2Id[BehaviorUnit.Signal]));
    ...
    private void OnSignalOptionItemSelected(long index)
    {
        ...
        BehaviorUnit.Signal = Core.Behavior.SignalId2Name[id];
    }
